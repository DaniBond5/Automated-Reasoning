from pysmt.shortcuts import LT, And, Equals, Implies, Int, Ite, Not, Or, Plus, Real, Solver, Symbol
...
for s1 in suspects:
    for s2 in suspects:
        for s3 in suspects:
            assertions.append(Implies(And(richer(s1, s2), richer(s2, s3)), richer(s1, s3)))
...
for suspect in suspects:
    if suspect is not butler:
        ...
        assertions.append(Not(hates(agatha, suspect)))
...
for s1 in suspects:
    assertions.append(LT(Plus(Ite(hates(s1, s2), Real(1), Real(0)) for s2 in suspects), Real(3)))
# Equivalent:
# for s1 in suspects:
